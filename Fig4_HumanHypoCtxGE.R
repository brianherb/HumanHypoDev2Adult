...
#AllGE4K.integrated = readRDS(url('https://data.nemoarchive.org/biccn/grant/u01_devhu/kriegstein/transcriptome/scell/10x_v2/human/processed/analysis/Herb_2022_Hypothalamus/SeuratObj/GW18_GW20_34AllGE4K_2D_integrated_mt10n500.rds'))

AllGE4K.integrated = readRDS('./SeuratObj/GW18_GW20_34AllGE4K_2D_integrated_mt10n500.rds')
...
#AllGE4K_3D.integrated = readRDS('./SeuratObj/GW18_GW20_34AllGE4K_3D_integrated_mt10n500.rds') 
...
## write out metadata for Sup table 20
...
saveRDS(plot.data,file="./Analysis/All_CtxHyGE_Clustering_mt10n500_3D_PseudoBulkGroupsComb.rds")

p=plot_ly(data = plot.data, x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, color = ~PseudoBulkGroupsComb,colors = hue_pal()(length(unique(plot.data$PseudoBulkGroupsComb))),type = "scatter3d", mode= "markers",  marker = list(size = 0.2, width=0.2))
...
#reticulate::conda_install('r-reticulate', 'python-kaleido',envname='/local/projects-t3/idea/bherb/software/conda_env/r-4.1.0')

#kaleido(p,file="./TestPlots/All_CtxHyGE_Clustering_mt10n500_3D_PseudoBulkGroups.png")

#kaleido(p,file="~/Documents/UMB_work/Ament/BRAIN_init/Kriegstein500k/Hypo/Paper/Rewrite/Revision/All_CtxHyGE_Clustering_mt10n500_3D_PseudoBulkGroups.png")



p=plot_ly(data = plot.data, x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, color = ~PseudoBulkGroupsComb,colors = hue_pal()(length(unique(plot.data$PseudoBulkGroupsComb))),type = "scatter3d", mode= "markers",  marker = list(size = 0.2, width=0.2) ) 

p = p %>% layout(scene = list(camera = list( eye =list(  x = -1.25,y = 1.25, z = 1.25 ),center = list(x = 0,  y = 0, z = 0))))


plotly::save_image(p, "./TestPlots/All_CtxHyGE_Clustering_mt10n500_3D_PseudoBulkGroupsComb.pdf")

## rotates, but resolution is worse 



#test = signup(username='brianrherb', email='bherb@som.umaryland.edu', save = FALSE)


#plotly_IMAGE(p, format = "png", out_file = "./TestPlots/All_CtxHyGE_Clustering_mt10n500_3D_PseudoBulkGroups.png") 
...
AllGE4K.integrated$SeuratClusters = AllGE4K_3D.integrated@meta.data[colnames(AllGE4K.integrated),'integrated_snn_res.0.5']
...
pdf(paste('./TestPlots/All_CtxHyGE_mt10n500_BrainRegions_Groups_2D_noRaster.pdf',sep=''),width=12,height=8)
for(j in c("Phase","SeuratClusters","BrainRegion","PseudoBulkGroups","PseudoBulkGroupsComb")){
print(DimPlot(AllGE4K.integrated, reduction = "umap", group.by = j, label = TRUE, repel = TRUE,raster=FALSE,shuffle=TRUE))
}
dev.off()
...
### check non-dividing precursors  - why is this just G1? 
...
Idents(AllGE4K_3D.integrated.g1) = 'PseudoBulkGroups' #'PseudoBulkGroupsComb'?
...
## add in COL1A2, DLL1, LGALS1, and NEUROD2(?) EOMES, MYO10, CALD1, and VIM, RBP1,  PPP1R17 FAT3, NAV2


pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_AdditionalMarkers_BulkHeatmap_minZero.pdf',width=8,height=16)
plotBulkHeatmap(AllGE4K_3D.integrated.g1,c('COL1A2','DLL1','LGALS1','NEUROD2','EOMES','MYO10','CALD1','VIM','RBP1','PPP1R17','FAT3','NAV2'))
dev.off()


gn3 = c(gn[1:5],'VIM','MYO10','LGALS1','SOX2','CALD1','DLL1','EOMES','COL1A2','PPP1R17','PAX6','ASCL1','NAV2','FAT3',gn[7:29],'RBP1')


pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision.pdf',width=8,height=16)
plotBulkHeatmap(AllGE4K_3D.integrated.g1,gn3)
dev.off()


pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision_noVim.pdf',width=8,height=16)
plotBulkHeatmap(AllGE4K_3D.integrated.g1,gn3[-6])
dev.off()

# gn3=c("HES1","EGR1","ZFP36L1","JUNB","SOX9","VIM","MYO10","LGALS1","SOX2","CALD1","DLL1","EOMES","COL1A2","PPP1R17","PAX6","ASCL1","NAV2","FAT3","DLX2","NHLH1","SATB2","NEUROD2","BHLHE22","PPP1R1B","ZNF385D","TBR1","GAS7","NR4A2","DLX1","PROX1","NR2F2","ZBTB16","LHX6","MAF","SOX6","ARX","ISL1","ZFHX3","TSHZ2","ZFHX4","PEG3","RBP1")


pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision_noVimRbp1.pdf',width=8,height=16)
plotBulkHeatmap(AllGE4K_3D.integrated.g1,gn3[-c(6,42)])
dev.off()


### try capping expression in data? 



 plotBulkHeatmap2 <- function(sobj = sobj,genes = genes,maxExp=NA){
...
pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision_maxExp15.pdf',width=8,height=16)
plotBulkHeatmap2(sobj=AllGE4K_3D.integrated.g1,genes=gn3,maxExp=15)
dev.off()


## all phases 

AllGE4K_3D.integrated.allPh = subset(AllGE4K_3D.integrated,cells=colnames(AllGE4K_3D.integrated)[which(AllGE4K_3D.integrated$PseudoBulkGroupsComb%in%c('RadialGlia','EarlyNeuPro','LateExcNeuPro_Ctx','LateInhibNeuPro','ExcEtNeu','ExcItNeu','CALB2pos-InhibNeu','FOXP1pos-InhibNeu','LHX6pos-InhibNeu'))])

DefaultAssay(AllGE4K_3D.integrated.allPh)='RNA'

Idents(AllGE4K_3D.integrated.allPh) = 'PseudoBulkGroups' #'PseudoBulkGroupsComb'?

plotGr = unique(AllGE4K_3D.integrated.allPh$PseudoBulkGroups)[c(3,17,9,7,20,10,13,5,8,12,1,18,15,4,19,14,6,21,11,2,22,16,23)]

AllGE4K_3D.integrated.allPh@active.ident=factor(AllGE4K_3D.integrated.allPh$PseudoBulkGroups,levels=plotGr)


pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision_maxExp15_allPhase.pdf',width=8,height=16)
plotBulkHeatmap2(sobj=AllGE4K_3D.integrated.allPh,genes=gn3,maxExp=15)
dev.off()

pdf('./TestPlots/All_CtxHyGE_mt10n500__NeuronLin_TopMarkers_BulkHeatmap_minZero_Revision_allPhase.pdf',width=8,height=16)
plotBulkHeatmap(sobj=AllGE4K_3D.integrated.allPh,genes=gn3)
dev.off()
## pretty much the same w/o isolating G1


## Ascl1 violin 

pdf(paste("./TestPlots/All_CtxHyGE_mt10_ASCL1_Violin.pdf",sep=''),height=4,width=16)

print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'ASCL1', split.by = 'BrainRegion',cols=c('red','green','blue')))

dev.off()


pdf(paste("./TestPlots/All_CtxHyGE_mt10_PPP1R17_Violin.pdf",sep=''),height=4,width=16)

print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'PPP1R17', split.by = 'BrainRegion',cols=c('red','green','blue')))

dev.off()



pdf(paste("./TestPlots/All_CtxHyGE_mt10_TFcheck_Violin.pdf",sep=''),height=4,width=16)

print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'CSRNP3', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'ARID4A', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'PEG3', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'BASP1', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'SCAND1', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'ZNF428', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'YBX1', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'PTMA', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'TCF4', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'LHX6', split.by = 'BrainRegion',cols=c('red','green','blue')))
print(VlnPlot(object = AllGE4K_3D.integrated.allPh, features = 'ARX', split.by = 'BrainRegion',cols=c('red','green','blue')))

dev.off()



## check DLX1, DLX2, DLX5 and DLX6 in GE 

AllGE4K_3D.integrated.GE = subset(AllGE4K_3D.integrated,cells=colnames(AllGE4K_3D.integrated)[which(AllGE4K_3D.integrated$BrainRegion=='GE')])


AllGE4K_3D.integrated.GE$GEregion='MGE'

AllGE4K_3D.integrated.GE@meta.data[grep('LGE',AllGE4K_3D.integrated.GE$sample),'GEregion'] = 'LGE'

AllGE4K_3D.integrated.GE@meta.data[grep('CGE',AllGE4K_3D.integrated.GE$sample),'GEregion'] = 'CGE'


DefaultAssay(AllGE4K_3D.integrated.GE)='RNA'

Idents(AllGE4K_3D.integrated.GE) = 'GEregion' 


pdf(paste("./TestPlots/JustGE_CtxHyGE_mt10_DLX_genes_Violin.pdf",sep=''),height=4,width=16)

print(VlnPlot(object = AllGE4K_3D.integrated.GE, features = 'DLX1'))
print(VlnPlot(object = AllGE4K_3D.integrated.GE, features = 'DLX2'))
print(VlnPlot(object = AllGE4K_3D.integrated.GE, features = 'DLX5'))
print(VlnPlot(object = AllGE4K_3D.integrated.GE, features = 'DLX6'))

dev.off()


testGE = FindAllMarkers(AllGE4K_3D.integrated.GE,features=c('DLX1','DLX2','DLX5','DLX6'))


### DEG's across Maj groups 

library(future)
plan("multiprocess", workers = 8)
options(future.globals.maxSize = 200000 * 1024^2)

MajGrpDEG = FindAllMarkers(AllGE4K_3D.integrated.g1)

saveRDS(MajGrpDEG,file='./Analysis/CtxHyGE_mt10_PseudoBulkGroups_DEGs.rds')

MajGrpDEGsig = MajGrpDEG[which(MajGrpDEG$p_val_adj<=0.05),]

write.csv(MajGrpDEGsig, file='./Analysis/CtxHyGE_mt10_PseudoBulkGroups_DEGs_Sig.csv',quote=FALSE)
...
gene = 'ASCL1'
 ...
 MajGrpDEG[which(MajGrpDEG$gene==gene),] 


summary(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='Hypo')])

length(which(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='Hypo')]>0))

summary(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='GE')])

length(which(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='GE')]>0))

summary(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='Cortex')])

length(which(AllGE4K_3D.integrated.g1@assays$RNA@counts[gene,which(AllGE4K_3D.integrated.g1@meta.data$BrainRegion=='Cortex')]>0))



## plot ASCL1 and NEUROD2 

plot.data <- FetchData(object = AllGE4K_3D.integrated, vars = c("UMAP_1", "UMAP_2", "UMAP_3", "rna_ASCL1","rna_NEUROD2"))

p=plot_ly(data = plot.data, x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, color = ~rna_NEUROD2,colors='Blues',type = "scatter3d", mode= "markers",  marker = list(size = 1, width=1))

htmlwidgets::saveWidget(p, "./TestPlots/All_CtxHyGE_Clustering_mt10n500_3D_rna_NEUROD2.html")

## try  colorscale = c('#FFE1A1', '#683531')
